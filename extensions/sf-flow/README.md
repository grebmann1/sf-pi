...
Advanced public-safe fixtures live under `scripts/e2e/fixtures/sf-flow-advanced/`. Run `npm run e2e:sf-flow-advanced -- --org <dedicated-non-production-alias>` for local diagnosis and combined check-only validation, `--deploy` to retain inactive Draft Flow and Apex fixtures, or `--runtime` to temporarily activate the Flow fixtures and run targeted Apex integration tests. The runtime sweep proves related-record creation, `$Record__Prior` on 1- and 200-record before-save and after-save updates, invocable Apex input/output cardinality and order, collection Transform execution, Collection Filter and Sort behavior, and primitive plus record-collection Subflow contracts. Runtime paths cover no-trigger autolaunched, before-save, after-save, before-delete, schedule-triggered, and platform-event Flows, including 200-record and 200-event batches. The scheduled path creates two bounded Accounts and four Contacts, waits for both related Tasks in the activating user’s time zone, and then removes the records and scheduled job. Cleanup runs in `finally`, deactivates every Flow with `activeVersionNumber=0`, and verifies zero fixture Account, Contact, Task, and CronTrigger residue.
